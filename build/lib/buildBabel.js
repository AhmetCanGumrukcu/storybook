...
export function buildWithWebpack(destination, componentName, componentEntry, ignoredFiles) {
    return exec(`node ./node_modules/webpack/bin/webpack --config webpack-transform.config.js --env.componententry="${componentEntry}" --env.componentname=${componentName} --env.distpath="${destination}" --env.ignoredFiles=${ignoredFiles} --bail`);
}
